    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="flex flex-col" x-data="{ expanded: false }">
            ...
            @foreach ($category->news as $index => $news)
                ...
                <article class="flex flex-col gap-2 pb-5 mb-5 border-b border-gray-100 last:border-none last:mb-0 last:pb-0 group">
            ...
            @if($category->news->count() >= 2)
                <a href="{{ route('kategori.show', $category->slug) }}" class="mt-2 self-start px-4 py-1.5 border border-green-600 text-green-600 text-xs font-medium tracking-wide hover:bg-green-100 rounded-sm">
                    Lihat Selengkapnya
                </a>
            @endif
